...
class Weel extends React.Component {
  ...
  spin() {
    this.spinTimer = null;
    this.setState({ startAngle: Math.random()*10000});
  ...
  renderBtnText = () => {
    if (this.state.text !== "") {
      //console.log("btn text: ", this.state.text);
      return (
        <div>
          {this.renderNumber()}
        </div>
        ...
        <div className="roulette-container  align-self-start">
          ...
          {this.props.arr.length !== 0 ?
            <Button
              ...
              <h5 className="blink text-uppercase m-0">{userLanguage?'Spin the wheel!':'旋转轮盘'}</h5>
            </Button>
            :
            <Button
